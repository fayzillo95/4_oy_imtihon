...
export class PermissionService {
  ...
  async createPermission(data : Required<CreatePermissionDto>){
    ...
    const newPermission = await this.permissionModel.create({...data, actions : [data.actions]})
  ...
  async findPermissionByUserId(user_id: string) {
    ...
    if (!permissionUser) throw new NotFoundException("Permission by user_id not found !")
    return permissionUser;
  }

  async updatePermissionByUserId(user_id: string, models: Models[], data: Actions[]) {
  ...
  async updatePermissionByPermissionId(id: string, data: UpdatePermission) {
    ...
    if (data.model) permission.update({
      model: data.model
    })
    await permission.save()
  ...
  async deletePermissionByUserId(user_id: string) {
    ...
    if (!permissionExists[0]) throw new NotFoundException("Permission not found !")
